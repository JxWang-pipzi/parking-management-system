...
Page({
  data: {
    statusBarHeight: 20,
    locationText: '北京市 朝阳区',
    activeTab: 'distance',
    quickEntries: [
      { type: 'nearbyParking', label: '附近停车', iconText: '停', theme: 'green' },
      { type: 'coupon', label: '优惠券', iconText: '券', theme: 'orange' },
      { type: 'monthCard', label: '月卡中心', iconText: '月', theme: 'yellow' },
      { type: 'charge', label: '充电桩', iconText: '充', theme: 'blue' },
      { type: 'more', label: '更多', iconText: '···', theme: 'gray' }
    ],
    banners: [
  ...
  getLocation() {
    ...
    return new Promise(function (resolve) {
      ...
      wx.getLocation({
        ...
        fail: function (err) {
          var defaultLocation = { latitude: 30.5728, longitude: 104.0668 }
          that.setData({
  ...
  async loadParkingLots() {
    ...
    try {
      var res = await get('/parking-lots', {
        latitude: location.latitude,
        longitude: location.longitude
      })

      var banners = ['/images/parking_building.jpg', '/images/parking_underground.jpg', '/images/parking_outdoor.jpg']
      var list = (res.data || []).map(function (lot) {
        var validDistance = !!(lot.routeDistanceMeters || lot.routeDistanceText)
        var dist = lot.routeDistanceMeters || (typeof lot.distance === 'number' ? lot.distance : 0)
        var distText = lot.routeDistanceText || (validDistance ? dist + '米' : '未知距离')
  ...
  onLocationTap() {
    ...
    wx.chooseLocation({
        success: function (res) {
          var locationName = res.name || res.address || '北京市 朝阳区'
          that.setData({
