...
如果后端跑在你的电脑上，请把开发地址改成电脑在当前局域网里的 IPv4 地址，例如 `http://192.168.1.8:8080/api`。
...
globalData: {
  userInfo: null,
  token: null,
  baseUrl: 'http://192.168.1.8:8080/api' // 修改为你的电脑局域网地址
}
```
...
1. 手机和电脑连接同一个 Wi-Fi
2. 后端服务已经启动，并监听 `8080` 端口
3. Windows 防火墙允许 Java 或 `8080` 入站访问
